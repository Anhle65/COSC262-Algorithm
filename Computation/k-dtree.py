...
class KdTree:
    ...
    def __init__(self, points, depth=0, max_depth=10):
        ...
        """
        if depth >= max_depth: # Ensure at least one point per leaf
            self.is_leaf = True
            self.points = points
        else:
            self.is_leaf = False
    ...
    def points_in_range(self, query_rectangle):
        ...
        """
        bottom = query_rectangle[0]
        top = query_rectangle[1]
        matches=[]
        if self.is_leaf:
            matches = [point for point in self.points if point.in_box(bottom, top)]
        else:
            if self.axis == 0 and self.coord >= bottom.x:
                matches += self.leftorbottom.points_in_range(query_rectangle)
            if self.axis == 0 and self.coord <= top.x:
                matches += self.rightortop.points_in_range(query_rectangle)
            if self.axis == 1 and self.coord >= bottom.y:
                matches += self.leftorbottom.points_in_range(query_rectangle)
            if self.axis == 1 and self.coord <= top.y:
                matches += self.rightortop.points_in_range(query_rectangle)
        return matches
