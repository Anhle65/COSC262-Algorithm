...
def lcs(s1, s2):
    ...
    while i > 0 and j > 0:
        if s1[i - 1] == s2[j - 1]:
            ...
            j -= 1
    return deque(lcs_str)
def different_character(str, lcs_str):
    result = []
    lcs = deque(lcs_str)
    for char in str:
        if lcs and lcs[0] == char:
            lcs.popleft()
            result.append(char)
        else:
...
def line_edits(s1, s2):
    ...
    while i > 0 or j > 0:
        if i > 0 and j > 0 and lines1[i - 1] == lines2[j - 1]:
            ...
            lcs_deque = lcs(lines2[i - 1], lines1[j - 1])
            left = different_character(lines1[i - 1], lcs_deque.copy())
            right = different_character(lines2[j-1], lcs_deque)
